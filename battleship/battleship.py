...
class Ship(object):
    ...
    def place(self, col, row, direction):
        ...
        # vertical - all the same column so keep same col coordinate
        # increase row number by length of ship.
        if direction == "V":
            for x in range(row, (self._length + row)):
                coordinates.append((col, x))
        
        if direction == "H":
            for x in range(col, (self._length + col)):
                coordinates.append((x, row))
...
class Player(object):
    ...
    def handle_shot(self, col, row):
        ...
        Let's create a player and add a ship:
            ...
            >>> player._board[1][2] = destroyer
            >>> player._board[1][3] = destroyer
            >>> player._ships.append(destroyer)
        ...
        if not ship:
            ...
            if ship.is_sunk():
                ...
                # remove the ship from the players arsenal
    ...
    def is_dead(self):
        ...
        And destroy it (and it's their only ship, too1)
            ...
            True
        """
        # if our aresenal (list of ships) is empty, return true
