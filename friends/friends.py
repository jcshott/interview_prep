...
class FriendGraph(object):
    ...
    def are_connected(self, name1, name2):
        ...
        def _are_connected(current, to_find, seen):
            # we found our name2 as a connection, pass that up
            if current.name == to_find:
                return True

            # otherwise, add that we've seen this node so we don't check again
            # and end up in endless loop
            seen.append(current)
            # check all our adjacent nodes
            for x in current.adjacent:
                # as long as they aren't in our "seen" list
                if x not in seen:
                    return _are_connected(x, to_find, seen)
            # if we get through all the adjacent and don't find a match, send up the chain
            return False
        
        return _are_connected(self.nodes[name1], name2, [])
