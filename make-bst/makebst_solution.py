...
def make_bst(nums):
    ...
    #find mid-point number in list
    mid_index = len(nums)/2

    #initiate data attribute to the midpoint
    midpoint = nums[mid_index]

    #create node. data initiates at midpoint, L/R initiate at None.

    node = BinaryNode(midpoint)

    #use recursion to find left and right of the node. need to find the midpoint of the L/R side of the midpoint
    #this happend until we are calling function on empty list. then we can come "back up" the recursion hole. the node.left & node.right are now set.
